                ...
                if (mysqli_num_rows($result) == 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo '      <div class="col-md-6 alert alert-danger text-center" role="alert" style = "margin: 1%;">
                                        <div class="mb-4">';
                        echo '              <h3>Are you sure you want to delete the ' . $row['topic_subject'] . ' topic?</h3>';
                        echo '          </div>';
                        echo '          <div class="row">
                                            <div class="col">';
                        echo '                  <form method="POST" class="form-button" action="deleteconfirm-topic.php?id=' . $row['topic_cat'] . '"> 
                                                    <button type = "submit" class = "yes btn btn-primary btn-block mb-4" name = "yes" value= ' . $id . '>Yes </button> 
                                                </form>
                                            </div>';
                        echo '              <div class="col">
                                                <form method="POST" action="category.php?id= ' . $row['topic_cat'] . '"> 
                                                    <button type = "submit" class = "no btn btn-danger btn-block mb-4" name = "no" >No </button> 
                                                </form>
                                            </div>';
                        echo '          </div>';
                        echo '      </div>
