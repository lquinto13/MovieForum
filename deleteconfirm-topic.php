                ...
                if (mysqli_num_rows($result) == 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        if (mysqli_query($db, $sql)) {
                            echo "<div class='alert alert-success' role='alert' style='margin: 1%'>You have deleted the topic successfully.</div>";
                            echo '<div class="alert alert-primary" role="alert" style="margin: 1%">Return to  <a href="index.php?id=) ">Home Page</a>.</div>';
                        } else {
                            echo '  <div class="alert alert-danger" role="alert" style="margin: 1%">
							            <i class="fas fa-exclamation-circle"></i> Error deleting record: ' . mysqli_error($db) .
                                '   </div>';
                        }
                    }
                }
